#!/usr/bin/env python3
"""使用 ChEMBL API 為未映射成分建立補充映射

此腳本會：
1. 載入目前未映射的成分清單
2. 使用 ChEMBL API 查詢每個成分
3. 找出與 DrugBank 匹配的同義詞
4. 輸出可加入 drugbank_mapper.py 的映射對照
"""
...
from twtxgnn.mapping.chembl_bridge import ChEMBLBridge
...
def main():
    ...
    # 檢查是否使用本地 SQLite
    use_sqlite = os.environ.get("CHEMBL_USE_SQLITE", "false").lower() == "true"
    ...
    print(f"使用模式: {'本地 SQLite' if use_sqlite else 'API'}")
    ...
    # 使用 ChEMBL 查詢
    bridge = ChEMBLBridge(use_sqlite=use_sqlite)
    found_mappings = {}
    not_found = []

    # 從環境變數讀取查詢數量限制
    max_queries = int(os.environ.get("CHEMBL_MAX_QUERIES", 200))
    ...
    for i, ingredient in enumerate(candidates[:max_queries]):
        ...
        if match:
            found_mappings[ingredient_upper] = match
            logger.info(f"找到映射: {ingredient_upper} -> {match}")
        else:
    ...
    if found_mappings:
        ...
        # 儲存到檔案
        output_file = Path("data/processed/chembl_mappings.json")
