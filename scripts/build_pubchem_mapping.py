#!/usr/bin/env python3
"""使用 PubChem API 為未映射成分建立補充映射

此腳本會：
1. 載入目前未映射的成分清單
2. 使用 PubChem API 查詢每個成分的同義詞
...
from twtxgnn.mapping.pubchem_bridge import PubChemBridge
...
def main():
    ...
    # 從環境變數讀取查詢數量限制
    max_queries = int(os.environ.get("PUBCHEM_MAX_QUERIES", 200))
    ...
    if found_mappings:
        ...
        # 儲存到檔案
        output_file = Path("data/processed/pubchem_mappings.json")
