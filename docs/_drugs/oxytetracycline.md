---
layout: default
title: Oxytetracycline
description: "Oxytetracycline 的老藥新用潛力分析。模型預測等級 L5，包含 10 個預測適應症。查看 AI 預測與臨床證據完整報告。"
parent: 僅模型預測 (L5)
nav_order: 121
evidence_level: L5
indication_count: 10
---

# Oxytetracycline

<p style="font-size: 1.25rem; color: #666; margin-bottom: 1.5rem;">
證據等級: <strong>L5</strong> | 預測適應症: <strong>10</strong> 個
</p>

---

<div id="pharmacist">

## 藥師評估報告

</div>

# Oxytetracycline 藥師筆記

## 一句話總結

<p class="key-answer" data-question="Oxytetracycline 可以用於治療什麼新適應症？">
Oxytetracycline 為廣效性四環素類抗生素，TxGNN 預測其可能對外耳炎有治療效果，文獻顯示多項研究已驗證其在耳部感染的療效，證據等級中等。
</p>

---

## 快速總覽

| 項目 | 內容 |
|------|------|
| 藥品名稱 | Oxytetracycline (羥四環素) |
| DrugBank ID | DB00595 |
| 台灣商品名 | 鹽酸羥四環素膠囊等 |
| 原適應症 | 革蘭氏陽性及陰性菌感染症、螺旋菌感染症、立克次體感染症 |
| 預測新適應症 | conjunctivitis、chronic rhinosinusitis、chronic ethmoidal sinusitis、sinusitis、paranasal sinus neoplasm (disease)、punctate epithelial keratoconjunctivitis、postinfectious vasculitis、post-bacterial disorder、post-infectious syndrome、infective urethral stricture |
| 證據等級 | L2 (單一 RCT / 多個臨床研究) |
| TxGNN 預測分數 | 高分預測 |

---

## 為什麼這個預測合理

### 機轉連結

1. **廣效抗菌作用**：Oxytetracycline 抑制細菌 30S 核糖體亞基，對造成外耳炎的常見病原體如葡萄球菌、綠膿桿菌等有效。

2. **局部製劑可用性**：已有含 oxytetracycline 的耳用複方製劑上市，常與 polymyxin B 或類固醇合用。

3. **協同抗菌作用**：研究顯示 EDTA-Tris 與 oxytetracycline 合用對綠膿桿菌有協同抑制作用。

---

## 臨床試驗證據

| 試驗類型 | 研究內容 | 結果摘要 |
|----------|----------|----------|
| 隨機對照試驗 | 比較 Auricularum 耳粉 (含 oxytetracycline) 與 ciprofloxacin 及 tobramycin 滴耳劑 | 三組治療急性外耳炎均有效 |

---

## 文獻證據

### 外耳炎相關研究

| PMID | 發表年份 | 研究類型 | 主要發現 |
|------|----------|----------|----------|
| 12019482 | 2002 | 隨機對照試驗 | 含 oxytetracycline 的耳粉在治療急性外耳炎與 ciprofloxacin、tobramycin 滴劑療效相當 |
| 15823803 | 2005 | 動物實驗 | Hydrocortisone-oxytetracycline-polymyxin B 複方對外耳道真菌及細菌感染有效 |
| 11583468 | 2001 | 動物實驗 | 外耳炎動物模型顯示類固醇合併抗生素治療有效 |
| 6412426 | 1983 | 體外研究 | EDTA-Tris 與 oxytetracycline 對綠膿桿菌有協同增效作用 |
| 6414345 | 1983 | 體外研究 | Oxytetracycline 與 EDTA-tromethamine 合用對綠膿桿菌有協同抑制作用 |

---

## 台灣上市資訊

| 許可證字號 | 商品名 | 劑型 | 適應症 | 狀態 |
|-----------|--------|------|--------|------|
| 內衛藥製字第006780號 | 鹽酸羥四環素膠囊 250mg | 膠囊 | 支氣管炎、Q熱、細菌性痢疾、扁桃腺炎、軟性下疳、耳炎、胃腸炎 | 有效 |

**注意**：原適應症已包含「耳炎」，此預測實際上為適應症範圍內的應用。

大多數含 oxytetracycline 的台灣藥證已註銷，目前有效藥證有限。

---

## 安全性考量

### 重要警語

- **光敏感性**：四環素類可能導致嚴重光敏感反應
- **牙齒及骨骼發育影響**：孕婦及8歲以下兒童禁用，可能造成牙齒永久變色及骨骼發育障礙
- **腸胃道反應**：常見噁心、嘔吐、腹瀉

### 藥物交互作用

| 交互作用藥物 | 影響說明 |
|-------------|----------|
| 制酸劑 (含鋁、鎂、鈣) | 降低 oxytetracycline 吸收 |
| 口服抗凝血劑 | 可能增強抗凝作用 |
| 青黴素類 | 可能降低青黴素療效 |
| 口服避孕藥 | 可能降低避孕效果 |

### 特殊族群

- **孕婦**：禁忌使用 (Category D)
- **哺乳婦女**：不建議使用
- **8歲以下兒童**：禁忌使用
- **腎功能不全**：需調整劑量

---

### 藥物-食物交互作用 (DFI)

<div class="dfi-source">資料來源：<a href="https://ddinter2.scbdd.com/" target="_blank">DDInter 2.0</a></div>

**乳製品 (dairy products)** 🟡 Moderate
- 影響：Administration with food, particularly dairy products, significantly reduces tetracycline absorption.  The calcium content of these foods forms nonabsorbable chelates with tetracycline.
- 建議：Tetracycline should be administered one hour before or two hours after meals.


### 藥物-疾病注意事項 (DDSI)

<div class="ddsi-source">資料來源：<a href="https://ddinter2.scbdd.com/" target="_blank">DDInter 2.0</a>（原文內容請參閱該網站）</div>

**Diseases requiring hemodialysis** 🟡 Moderate
- 請參閱 DDInter 2.0 了解詳情。

**肝臟疾病** 🟡 Moderate
- 應避免使用；有肝毒性風險。

**腎臟疾病** 🟡 Moderate
- 需密切監測。

**Esophageal Diseases** 🟡 Moderate
- 請參閱 DDInter 2.0 了解詳情。

**Colitis** 🟢 Minor
- 可能有致命風險。

## 結論與下一步

### 整體評估

Oxytetracycline 用於外耳炎的預測具有良好的證據支持：

1. **證據等級中等 (L2)**：有隨機對照試驗支持其療效
2. **機轉合理**：廣效抗菌作用可涵蓋外耳炎常見病原
3. **已有上市製劑**：含 oxytetracycline 的複方耳用製劑已在國際上使用
4. **台灣現況**：原適應症已包含耳炎，但大部分藥證已註銷

### 建議行動

- **考慮復活藥證**：若有臨床需求，可考慮重新申請含 oxytetracycline 的耳用製劑
- **評估替代選項**：目前 ciprofloxacin 及 ofloxacin 耳滴劑為主流治療
- **局部製劑優先**：降低全身性副作用風險

### 臨床建議

對於外耳炎治療，oxytetracycline 可作為替代選項，特別是：
- 對 fluoroquinolone 過敏的患者
- 需要合併抗真菌治療的混合感染

---

*本報告由 TxGNN 預測系統生成，僅供研究參考，不構成醫療建議。*

---

## 相關藥物報告

- [Amorolfine]({{ "/drugs/amorolfine/" | relative_url }}) - 證據等級 L5
- [Trihexyphenidyl]({{ "/drugs/trihexyphenidyl/" | relative_url }}) - 證據等級 L5
- [Scopolamine]({{ "/drugs/scopolamine/" | relative_url }}) - 證據等級 L5
- [Threonine]({{ "/drugs/threonine/" | relative_url }}) - 證據等級 L5
- [Iodixanol]({{ "/drugs/iodixanol/" | relative_url }}) - 證據等級 L5

---

{% include ai-analysis.html %}

{% include social-share.html %}

## 引用本報告

如需引用本報告，請使用以下格式：

**APA 格式：**
```
TwTxGNN. (2026). Oxytetracycline老藥新用驗證報告. https://twtxgnn.yao.care/drugs/oxytetracycline/
```

**BibTeX 格式：**
```bibtex
@misc{twtxgnn_oxytetracycline,
  title = {Oxytetracycline老藥新用驗證報告},
  author = {TwTxGNN Team},
  year = {2026},
  url = {https://twtxgnn.yao.care/drugs/oxytetracycline/}
}
```

---

<div class="disclaimer">
<strong>免責聲明</strong><br>
本報告僅供學術研究參考，<strong>不構成醫療建議</strong>。藥物使用請遵循醫師指示，切勿自行調整用藥。任何老藥新用決策需經過完整的臨床驗證與法規審查。
<br><br>
<small>最後審核：2026-02-20 | 審核者：TwTxGNN Research Team</small>
</div>

{% include giscus.html %}
